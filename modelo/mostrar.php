...
class Mostrar
{
    ...
    // public static function insertarAlumnos($conn, $nombre, $apellido, $fechaNac, $telefono, $dni, $email, $foto, $password1)
    // {
    //     try {
    //         $sql = "INSERT INTO usuarios (nombre, apellido, fecha, telefono, dni, email, foto, password1, permiso, descripcion) " .
    //             "VALUES (:nombre, :apellido, :fecha, :telefono, :dni, :email, :foto, :password1, :permiso, :descripcion)";
    //         $resultado = $conn->prepare($sql);
    //         $resultado->bindValue(':nombre', $nombre);
    //         $resultado->bindValue(':apellido', $apellido);
    //         $resultado->bindValue(':fecha', $fechaNac);
    //         $resultado->bindValue(':telefono', $telefono);
    //         $resultado->bindValue(':dni', $dni);
    //         $resultado->bindValue(':email', $email);
    //         $resultado->bindValue(':foto', $foto);
    //         $resultado->bindValue(':password1', $password1);
    //         $resultado->bindValue(':permiso', '2');
    //         $resultado->bindValue(':descripcion', 'alumno');
    //         $resultado->execute();
    //         return true;
    //     } catch (Exception $e) {
    //         return "Se produjo un error. Código: " . $e->getCode() . ", " . $e->getMessage(); //Guardamos en la variable el error producido.
    ...
    // public static function selectFacturas($conn, $idfactura)
    // {
    //     try {
